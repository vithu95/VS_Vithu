...
public class ManufacturerServer3 implements MqttCallback, Runnable {

    File allsensors = new File("ManufacturerServer3//Logging//logger.txt");
    ...
    public void run() {
        ...
        try {
            //FileWriter writer_table = new FileWriter(allsensors.getAbsoluteFile(), true);
            new ManufacturerServer3().subscribe("Server");
        } catch (InterruptedException ex) {
            Logger.getLogger(ManufacturerServer3.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    MqttClient mqttClient;

    public ManufacturerServer3() {
        try {
...
/*
    public void addWeatherData(String m) {
        try {
            String idd;
            System.out.println(m);
            if (m.contains(" ")) {
                idd = m.substring(0, m.indexOf(" "));
                m = m.substring(m.indexOf(" ") + 2);
                System.out.println("X" + m);

                for (int i = m.indexOf(" "); i != -1; ) {
            ...
            //System.out.println(api_data.get("temperature"));
        } catch (Exception e) {
            System.out.println(e);
        }
    }
